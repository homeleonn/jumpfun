...
class Captcha{
	...
	public function set(){
		...
		*/
		...
		$_SESSION['captcha_code'] = implode("",$chars); // передаем в сессию значение капчи для последующей проверки
		imagepng($img); exit;// создаем изображение (png)
	}
	
	public static function validation(){
		//dd(session('captcha_code'), $_POST['captcha_code']);
		return isset($_POST['captcha_code']) && session('captcha_code') == $_POST['captcha_code'];
	}
}