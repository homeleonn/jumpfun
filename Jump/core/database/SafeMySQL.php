...
class SafeMySQL
{
	...
	{
		$opt = $this->defaults = array_merge($this->defaults,$opt);
	...
	function q($query, $mode = null, $esc = null)
	{
		global $db;
		if(!$db) $db = lazyConnect();
		$conn = $db->getConn();
		if($esc !== null) $query = str_replace('?s', esc($conn, $esc), $query);
		$modes = array('assoc', 'num', 'row', 'one');
		if(in_array($mode, $modes))
		{
			$return = array();
			$res = queryTrace($conn, $query);
			if(!$res)
			{
				exit(trigger_error(mysqli_error($conn).' Полный запрос: '.$query,E_USER_ERROR));
				return false;
			}
			if($mode == 'row') return mysqli_fetch_assoc($res);
			if($mode == 'one'){
				$res = mysqli_fetch_row($res);
				return $res[0];
			}
			$mode = ($mode == 'assoc')? MYSQLI_ASSOC : MYSQLI_NUM;
			while($ret = mysqli_fetch_array($res, $mode)){
				$return[] = $ret;
			}
	...
	function esc($conn, $s, $query = NULL){
		
		
		return '\'' . mysqli_real_escape_string($conn, $s) . '\'';
	}
	
	function queryTrace($conn, $query){
		$start = microtime(TRUE);
		$res = mysqli_query($conn, $query);
		$timer = microtime(TRUE) - $start;
		global $savedQuery;$savedQuery[] = array('query' => $query, 'start' => $start, 'timer' => $timer);
		return $res;
	}
	
	function getQStats(){
		global $db, $savedQuery;
		if(!$db) return 'База данных не была задействована!';
		return array($db->getStats(), $savedQuery);
	}
	
	function lazyConnect(){
		global $params;
		return new SafeMySQL($params);
	}
